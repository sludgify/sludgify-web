    ...
    return (
        <div className="flex min-h-screen w-screen flex-col">
            ...
            <main className="flex-1 w-full">
                {/* Hero Section*/}
                <div className="bg-[url('/bg-sustainability-report.svg')] text-secondary w-full bg-no-repeat bg-cover bg-center  h-[300px] md:h-[690px] md:space-y-3 flex flex-col justify-center">
                    <RevealOnScroll className="flex flex-col justify-center gap-3 p-8 md:space-y-3 md:p-20" delay={200}>
                        <h1 className="text-2xl md:text-6xl font-radley">Embedding Sustainability into Core Business Strategy</h1>
                ...
                {/* Core Value Section */}
                <div className="font-radley w-screen p-6 md:p-12 flex flex-col items-center justify-evenly gap-8">
                    <RevealOnScroll delay={150}>
                        <h1 className="text-xl md:text-6xl font-radley text-primary text-center md:w-[810px]">Unlock Value Through Sustainability Reporting</h1>
                    </RevealOnScroll>
                    <div className="grid grid-cols-1 md:grid-cols-3 md:gap-8">
                        {services.map((service, index) => (
                            <RevealOnScroll key={index} delay={index * 100}>
                                <div className="mb-8 break-inside-avoid bg-white p-6 md:w-[396px] inline-block">
                                    <Image src={service.url} alt={service.title} width={396} height={423} className="md:mb-4 w-[325px] md:w-[396px] h-[306px] md:h-[423px] rounded-[10px] object-cover" />
                                    <h2 className="text-xl md:text-4xl font-radley md:mb-4">{service.title}</h2>
                                    <p className="text-[10px] md:text-xl text-[#505050]">{service.desc}</p>
                                </div>
                            </RevealOnScroll>
                        ))}
                    </div>
                ...
                {/* Client Section */}
                <div className="w-full bg-white overflow-hidden hide-scrollbar px-4 py-12 flex flex-col items-center gap-4 md:gap-8 overflow-x-visible relative">
                    <RevealOnScroll delay={150}>
                        <h1 className="text-xl md:text-6xl font-radley text-primary text-center">What Our Client Say</h1>
                    </RevealOnScroll>
                    <RevealOnScroll delay={150}>
                        <p className="text-[#525252] text-[10px] md:text-lg max-w-[279px] md:max-w-[1000px] text-center">
                                ...
                                <CarouselContent>
                                    {clients.map((client) => (
                                        <CarouselItem key={client.id} className="md:basis-1/3 basis-1/2">
                                            <div className="bg-white drop-shadow-lg border border-[#D9D9D9] rounded-lg w-[207px] h-[320px] md:w-[590px] md:h-[692px] p-3 md:p-5 flex flex-col gap-4">
                                                <Image src={client.banner} alt={`${client.company} banner`} width={600} height={200} className="object-cover md:w-[600px] w-[183px] md:h-[417px] rounded" />
                                                ...
                                                <div className="flex items-center mt-auto gap-4">
                                                    <Image src={client.image} alt={client.company} width={75} height={75} className="w-[30px] h-[30px] md:w-[75px] md:h-[75px] object-cover rounded-full" />
                                                    <div className="md:text-lg text-[10px] font-radley">
                ...
                <div className="container mx-auto px-10 py-20">
                    <RevealOnScroll delay={150}>
                        <h1 className="text-xl md:text-6xl font-radley text-center mb-12">Our approach to classify greenhouse gas emissions across institutions</h1>
                    </RevealOnScroll>
                    <RevealOnScroll delay={150}>
